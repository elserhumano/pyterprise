...
class Workspace(object):
    ...
    def list_resources(self):
        """
        Lista todos los recursos del workspace usando la API de Terraform
        GET /workspaces/:workspace_id/resources
        """
        resources_data = self._api_handler.call(
            method='get',
            uri=f'workspaces/{self.id}/resources'
        ).data
        ...
        for r in resources_data:
            resources.append(Resource(
                ...
                module=r['attributes']['module'],
                provider_type=r['attributes']['provider-type']
            ))
        return resources


    def list_resources2(self):
        """
        Lista todos los recursos del workspace usando la API de Terraform
        GET /workspaces/:workspace_id/resources
        ...
        """
        resources = []
        next_page_uri = f'workspaces/{self.id}/resources'
        ...
        while next_page_uri:
            ...
            # Revisa si hay otra página en la paginación
            next_page_uri = None
            pagination = response.meta.get('pagination') if hasattr(response, 'meta') else None
            if pagination and pagination.get('next-page'):
                next_page_uri = f"workspaces/{self.id}/resources?page[number]={pagination['next-page']}"
