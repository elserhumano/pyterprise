...
allow_orgs = [ 'My-Company-Networks',
               'My-Company-Networks-Shared'
             ]

check_variables = [ 'refresh_token',
                    'vra_refresh_token'
                  ]
    ...
    if org.name in allow_orgs:
        for varset in org.list_varsets():
            for variable in varset.list_set_variables():
                if variable.key in check_variables:
                #if variable.key == "vra_refresh_token":
                    # The dates to check
                    ...
                    #print ("Description variable: ",variable.description)
                    #if "_SE_ " in variable.description:
                        ...
                        if the_element not in list_accounts:
                            new_element = the_element + ' - ' + varset.name
                            list_accounts.append(new_element)
